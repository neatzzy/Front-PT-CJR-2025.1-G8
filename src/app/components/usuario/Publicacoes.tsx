          ...
          >
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-[#22a27a]">
                <Image
                  ...
                  alt={pub.usuario?.nome ? `Avatar de ${pub.usuario.nome}` : "Avatar"}
                  width={40}
                  height={40}
                />
              </div>
              <div>
                <span className="font-bold">{pub.usuario?.nome || "Usuário"}</span>
            ...
            <div className="flex items-center gap-4 text-gray-700 mt-1">
              <span className="flex items-center gap-1 text-base">
                <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" className="inline">
                  <circle cx="10" cy="10" r="8" />
                </svg>
                {Array.isArray(pub.comentarios) ? pub.comentarios.length : 0} comentários
              </span>
              {isOwner && (
                <>
                  <button className="ml-auto hover:text-[#179478]" onClick={() => handleEdit(pub.id)}>
                    <FaEdit />
                  </button>
                  <button className="hover:text-[#b94a4a]" onClick={() => handleDelete(pub.id)}>
                    <FaTrash />
                  </button>
                </>
              )}
